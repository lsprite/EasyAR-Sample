...
namespace samples {
    ...
    void AR::loadFromSDImage(const std::string &path, int tracker_id) {
        ImageTarget target;
        std::string jstr = "{\n"
                                   "  \"images\" :\n"
                                   "  [\n"
                                   ...
                                   "      \"image\" : \"" + path + "\",\n"
                                   "      \"name\" : \"" +
                           path.substr(path.find_last_of("/") + 1, path.find_first_of(".")) + "\"\n"
                                   "    }\n"
                                   "  ]\n"
                                   "}";
        //LOGI("dddddddddddd: %s (%d) %s\n", jstr.c_str(), 1, "");
        target.load(jstr.c_str(),
                    EasyAR::kStorageApp | EasyAR::kStorageAssets | EasyAR::kStorageJson);
        if (tracker_id == 0)
    ...
    void AR::setSweep(bool mIsSweep) {
        mIsSweep_ = mIsSweep;
    }
